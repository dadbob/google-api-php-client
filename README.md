...
The Appning API Client Library enables you to work with Appning APIs on your server.

This library is based on the Google API PHP Client and maintains compatibility with the original client's structure and features, adapted for use with Appning services.
...
For support with the library the best place to ask is via the appning-api-php-client tag on StackOverflow.
...
```
vendor/bin/phpcs src --standard=phpcs.xml.dist -np
```

To automatically fix (fixable) coding style violations, run

```
vendor/bin/phpcbf src --standard=phpcs.xml.dist
```
